...
//
  ...
  void instantlyLoadCSVwinCMD() async {
    ...
    if (isLoadingCSV == false && userAbortedLoadCSV == false) {  //readCsvFeautr (File('$argument0'));  // see void main, where were replaced all \-slashes to /-slashes in Path  (win-slashes to android-slashes)  //argument0 = "C:/Users/User/Desktop/Test.csv";  //   Right Slashes !!! Not Left !!!
      initNtState(rngEnd: rngEnd, isSwitched_32_64_128: isSwitched_32_64_128, mode_3264_or_64128: mode_3264_or_64128, nTcolS: nTcolS);
...
//     _audioRecorder.init();              // Initialize NEW recorder from audio_capture 2 of 4
    ...
    initNtState(rngEnd: rngEnd, isSwitched_32_64_128: isSwitched_32_64_128, mode_3264_or_64128: mode_3264_or_64128, nTcolS: nTcolS);   // was moved here from initStateFunctions.  //*** IF ASYNC FUNCTION WILL BE BEFORE THE SYNC FUNCTION, IT WILL RETURN RANDOM VALUES !!! SEE print(themeappLightDark) in getAppSettingsSharedPref()
...
///////////////////////////////////////////////  foreground/ background observer for android 3 of 3  ///////////////////////
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {        // after SDK upgrade it stops in background in Windows Too, earlier it was only in Android
    if(Platform.isAndroid || Platform.isIOS) {                      // stop playback if in background on Android, IOS. And do not stop if Windows, MacOS
      ...
      nt$.value.copyWith(msrTgl: 0);
          // nt$.value = ntTblNtfrsList;
...
//print(dataF);
    ...
    setState(() {
      ...
      isSwitched_32_64_128 = nt$.value.listener3264128!;
      mode_3264_or_64128 = nt$.value.listener326464128!;
    });
    initNtState(rngEnd: rngEnd, isSwitched_32_64_128: isSwitched_32_64_128, mode_3264_or_64128: mode_3264_or_64128, nTcolS: nTcolS);                                             //    Needed Only to swith view in common CSV/TSV-file After HTTP Google Spreadsheet Loading
    await loadTagsFirst();
...
//
  editorAutoplay() {      // editor autoplay:
    if (nt$.value.rangeStart?.round().remainder(nTcolS*2) != 0 || nt$.value.rangeEnd?.round().remainder(nTcolS*4) != 0) { //if Range Slider (Start or End) is not at the default position
      setState(() {toggleIcnMsrBtn = !toggleIcnMsrBtn;});
      nt$.value.copyWith(msrTgl: 1);
      // nt$.value = ntTblNtfrsList;
  ...
  MonitorFile() async{              // So far, manually cyclical monitoring
    ...
    if(willGoogleSpreadsheetBeLoaded != true) {
      if(csvMode == 1 && (playerMode == 1 || playerMode == 6)) {          //  "Android, MacOS, Linux"
        if(isSwitchedMonitorFile==true) {
          ...
          if (tempSMonitor!='') {
            initNtState(rngEnd: rngEnd, isSwitched_32_64_128: isSwitched_32_64_128, mode_3264_or_64128: mode_3264_or_64128, nTcolS: nTcolS); //////////////////// TRY 001
    ...
    if(isSliderResetNeeded == true) {
      if(isSwitchedMonitorFile==false) {
        initNtState(rngEnd: rngEnd, isSwitched_32_64_128: isSwitched_32_64_128, mode_3264_or_64128: mode_3264_or_64128, nTcolS: nTcolS);
        nt$.value.copyWith(rangeStart:  0); nt$.value.copyWith(rangeEnd:  nt$.value.nTcolS!*4);  nt$.value.copyWith(rngEndSlider:  rngEnd);
        // nt$.value = ntTblNtfrsList;    // resetting Range Slider, right handle - to the Right
        setState(() {});
      } else {} //end if file changes monitor
    } else {} // end if isSliderResetNeeded
    ///----------------------------------------------------------------------------------------------------------------------------------------------------
    switch (isSwitched_32_64_128) {
      case 32:  nt$.value.copyWith(captionVariant0: 0); break;   // '⬤〇〇'
      case 64:  nt$.value.copyWith(captionVariant0: 1); break;   // '〇⬤〇'
      case 128: nt$.value.copyWith(captionVariant0: 2); break;   // '〇〇⬤'
      default:  nt$.value.copyWith(captionVariant0: 1);          // '〇⬤〇'
...
//
  fromTheBeginBySliderCoincidence() {             // Suggest to Start Over (begin) by Slider Values Coincidence
    ...
    if(nt$.value.rangeStart?.round().remainder(nTcolS*2) == 0 && nt$.value.rangeEnd?.round().remainder(nTcolS*4) == 0){
      fromTheBegin = false;
    ...
    if(nt$.value.rangeStart?.round() == nt$.value.rangeEnd?.round() && nt$.value.rangeEnd?.round() != nTcolS*2 && nt$.value.tableChangeCount != 1){
      fromTheBegin = true;
    ...
    if(nt$.value.rangeStart?.round() == nt$.value.rangeEnd?.round() && nt$.value.rangeEnd?.round() == nTcolS*4){
...
///////////////////////////////////////////////////////////////////////////////////////////////////////    end TSV adding elements at the end   1 of 5
    setState(() {                                              // you dont have to write all assignments inside the setState(...)
      ...
      measureBtnTx = 'measures 1 - ' + (maxNotEmptyPos / nt$.value.nTcolS!).ceil().toString();            // nTcolS=32 is a cell layout in excel template
    });
...
//
    while ((data1.length).remainder(nt$.value.nTcolS!*2) != 0) { // adding an empty column until the total width of the two tables is a multiple of 64th
...
// for (var el in ntTblNtfrsList) {print(el.toString());}      // The BEST WAY TO PRINT ALL THE LIST !!!   //myDeveloper.log(el.toString());   // difference Print from Log is a Queue, Log will pass other messages between output    // (Log) shows element [0] is equal to Null, but (Print) still prints the element !!!  ??????  It's very strange!
    ...
    traversingForNavigate(csvLst, (nt$.value.tableChangeCount!*2-1) + 0); //CALL
...
//
  checkIfTCCareOutOfLimits() async {   // if previous session ends incorrectly or table change view ends incorrectly, grey screen prevention when file opening
    ...
    var tccLim = (maxNotEmptyPos / nt$.value.nTcolS!).ceil();
    if(tccLim.isOdd) {tccLim = tccLim + 1;} else {}
    if( ((nt$.value.tableChangeCount!*2)    > tccLim) ||
        (((nt$.value.tableChangeCount32!*2)  > tccLim) && nt$.value.listener3264128!  == 32) ||
        (((nt$.value.tableChangeCount64!*2)  > tccLim) && nt$.value.listener3264128!  == 64) ||
        (((nt$.value.tableChangeCount128!*2) > tccLim) && nt$.value.listener3264128!  == 128)
    ) {
      setState(() {
      ...
      initNtState(rngEnd: rngEnd, isSwitched_32_64_128: isSwitched_32_64_128, mode_3264_or_64128: mode_3264_or_64128, nTcolS: nTcolS); // needed!
      await setDataSharedPref();
...
//await preferences.clear();                                           				// CLEAR (APP SETTINGS) SHARED PREFERENCES ALL !!!
  ...
  Future<void> setDataSharedPref() async{
    ...
    var TCC128 = nt$.value.tableChangeCount128!;
    var TCC064 = nt$.value.tableChangeCount64!;
    var TCC032 = nt$.value.tableChangeCount32!;
    var PBIT   = nt$.value.playingBit!;
    var TCC    = nt$.value.tableChangeCount!;
    var SBIT   = nt$.value.startBit!;
    var rSTART = nt$.value.rangeStart!;
    var rEND = nt$.value.rangeEnd!;
    var STB128 = nt$.value.rangeStart128bak!;
    var ENB128 = nt$.value.rangeEnd128bak!;
    var STB064 = nt$.value.rangeStart64bak!;
    var ENB064 = nt$.value.rangeEnd64bak!;
    var STB032 =  nt$.value.rangeStart32bak!;
    var ENB032 = nt$.value.rangeEnd32bak!;
    //**********************************************************************//
    stringValSharedPref = '${TCC128.toString()}~${TCC064.toString()}~${TCC032
        ...
        .toString()}~${ENB032.toString()}~${nt$.value.listener3264128!.toString()}~${nt$.value.listener326464128!.toString()}';
...
//
  Future<void> clearOneFileSharedPrefByCondition(String crc32TPI) async {
    ...
    if(clearRangeSharePrefA == true) {
      ...
      nt$.value.copyWith(rangeStart:  0); nt$.value.copyWith(rangeEnd:  nt$.value.nTcolS!*4);  // resetting Range Slider
      setState(() {});} else {}
      await setDataSharedPref();
  }
...
//await preferences.clear();                                                                                                   // CLEAR SHARED PREFERENCES ALL !!!
    ...
    if (stringValSharedPref != '') {
      try {
        List<String> stLSP = stringValSharedPref.split("~");
        //**********************************************************************//
        nt$.value.copyWith(tableChangeCount128:  int.parse(stLSP[0]));
        nt$.value.copyWith(tableChangeCount64:   int.parse(stLSP[1]));
        nt$.value.copyWith(tableChangeCount32:   int.parse(stLSP[2]));
        nt$.value.copyWith(playingBit:           int.parse(stLSP[3]));
        nt$.value.copyWith(tableChangeCount:     int.parse(stLSP[4]));
        nt$.value.copyWith(startBit:             int.parse(stLSP[5]));
        nt$.value.copyWith(rangeStart:           int.parse(stLSP[6]));
        nt$.value.copyWith(rangeEnd:             int.parse(stLSP[7]));
        nt$.value.copyWith(rangeStart128bak:     int.parse(stLSP[8]));
        nt$.value.copyWith(rangeEnd128bak:       int.parse(stLSP[9]));
        nt$.value.copyWith(rangeStart64bak:      int.parse(stLSP[10]));
        nt$.value.copyWith(rangeEnd64bak:        int.parse(stLSP[11]));
        nt$.value.copyWith(rangeStart32bak:      int.parse(stLSP[12]));
        nt$.value.copyWith(rangeEnd32bak:        int.parse(stLSP[13]));
        nt$.value.copyWith(listener3264128:      int.parse(stLSP[14]));
        nt$.value.copyWith(listener326464128:    int.parse(stLSP[15]));
        // nt$.value = ntTblNtfrsList;
        setState(() {});
        //**********************************************************************//
      } catch(e){
...
//
  Future<void> increaseMeasureByButton() async { //  Measure Up by User
    ...
    requiredMeasToStop = (nt$.value.tableChangeCount!*2-1) + 0;
    if((nt$.value.tableChangeCount!*2-1) < (maxNotEmptyPos / nt$.value.nTcolS!).ceil() - 1) {requiredMeasToStop = (nt$.value.tableChangeCount!*2-1) + 2;} else {}
...
//
  Future<void> decreaseMeasureByButton() async { //  Measure Down by User
    ...
    nt$.value.copyWith(rangeStart:  0); nt$.value.copyWith(rangeEnd:  nt$.value.nTcolS!*4);  // resetting Range Slider
    // nt$.value = ntTblNtfrsList;
    int requiredMeasToStop; requiredMeasToStop = (nt$.value.tableChangeCount!*2-1) + 0;         // TABLE CHANGE COUNT CHECK IT !!!
    int requiredView;       requiredView = nt$.value.listener3264128!;
    toShowVLbls = false;                  // false  =  without showing big side labels
  //initStateFunctions(); //assertion error
    setState(() {
   //   // here: cause incorrect count, commented
      isSwitched_32_64_128 = nt$.value.listener3264128!;
      mode_3264_or_64128   = nt$.value.listener326464128!;
    });
    initNtState(rngEnd: rngEnd, isSwitched_32_64_128: isSwitched_32_64_128, mode_3264_or_64128: mode_3264_or_64128, nTcolS: nTcolS);            // instead of initState()  because  assertion error, but won't to startPlay at 1st view         resetting all TCC's to "1"
    isSwitched_32_64_128 = requiredView;
    nt$.value.copyWith(listener3264128:  requiredView);
    // nt$.value = ntTblNtfrsList;
...
// Do not use delay less than 0.001s or 1ms or 1000 us (microseconds)! Delay breakdown occurs!
  ...
  changeTableView(i,iStarts,dontChangeView) {  //changes view Only in UP direction, by "i"
    nt$.value.copyWith(onTapCollision1: 1); // nt$.value = ntTblNtfrsList;
    //**********************************************************************//  "Assembler-style" variables names:
    ...
    var TCC    = nt$.value.tableChangeCount!;
    var SBIT   = nt$.value.startBit!;
    var OisLEFT =  nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2);  // Range slider left Handle at the Left position
    var OisRIGHT = nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4);    // Range slider right Handle at the Right position
    var rSTART = nt$.value.rangeStart!;
    var rEND = nt$.value.rangeEnd!;
    var STB128 = nt$.value.rangeStart128bak!;
    var ENB128 = nt$.value.rangeEnd128bak!;
    ...
    var STB032 =  nt$.value.rangeStart32bak!;
    var ENB032 = nt$.value.rangeEnd32bak!;
    var ALLCOL = nt$.value.nTcolS!*2;
    var STARTr = rSTART?.round();
    //**********************************************************************//
    //
    if(nt$.value.listener3264128! ==128) {
      TCC=TCC128;
    ...
    } else if(nt$.value.listener3264128! ==64) {
      TCC=TCC064;
      if ((i - 1).remainder(ALLCOL) == 0 && i != 1 && TCC == 1 && STARTr != ALLCOL) {          						                                 // if (i - 1)  !!!
    ...
    } else if (nt$.value.listener3264128!  == 32) {
      TCC=TCC032;
      if ((i - 1).remainder(ALLCOL) == 0 && i != 1 && TCC == 1 && STARTr != ALLCOL) {         					                                 	  // if (i - 1)  !!!
        TCC032 = TCC032 + 1;
        TCC = TCC032;
...
//
//**********************************************************************//
    nt$.value.copyWith(tableChangeCount128: TCC128);
    nt$.value.copyWith(tableChangeCount64: TCC064);
    nt$.value.copyWith(tableChangeCount32: TCC032);
    nt$.value.copyWith(playingBit:  PBIT);
    nt$.value.copyWith(startBit:  SBIT);
    nt$.value.copyWith(tableChangeCount:  TCC);
    nt$.value.copyWith(rangeStart:  rSTART);
    nt$.value.copyWith(rangeEnd:  rEND);
    nt$.value.copyWith(rangeStart128bak:  STB128);
    nt$.value.copyWith(rangeEnd128bak:  ENB128);
    nt$.value.copyWith(rangeStart64bak:  STB064);
    nt$.value.copyWith(rangeEnd64bak:  ENB064);
    nt$.value.copyWith(rangeStart32bak: STB032);
    nt$.value.copyWith(rangeEnd32bak: ENB032);
    ////////////////////////////
ntTblNotifier.notifyListeners();                                                      // Try notifyListeners()  !!!
    // // nt$.value = ntTblNtfrsList;     // without setState()
    ////////////////////////////
//**********************************************************************//
//
    nt$.value.copyWith(onTapCollision1: 0); // nt$.value = ntTblNtfrsList;
  } // end changeTableView()
//
//
//
///////     NEW SOLUTION: USING TIMER IN THE SECOND "ISOLATE" 3 OF 3          (FOR SENDING DATA INTO THE MAIN ISOLATE WITH THE GUI)
  void _sendDataNDcommandToISO(int iStarts, int iEnds, List jBtnRelease, List csvLst, int notesByBit, bool rngExtend) async {
    ...
    List<dynamic> allData = [
      nt$.value,                    // NtState (NOT ValueNotifier!)
      iStarts,
      iEnds,
    ...
    if (_sendPortToPlayer != null && allData.isNotEmpty) {
...
//
   ...
   //    }
   // //
    _playerStream!.listen((message) async {
      if (message is String) {
        final parts = message.split(':');
        final key = parts[0];
        final value = parts.length > 1 ? parts[1] : null;
        final value2 = parts.length > 2 ? parts[2] : null;
        final value3 = parts.length > 3 ? parts[3] : null;
        ...
        switch (key) {
          ...
          case 'toggleIcnMsrBtn':
            final newValue = (value?.toLowerCase() == 'true');
          ...
          case 'msrTgl':
            ...
            if (nt$.value.msrTgl != newValue) {
              nt$.value = nt$.value.copyWith(msrTgl: newValue); // СОХРАНЯЕМ результат!
              developer.log('msrTgl_received: $newValue');
            }
            break;

          case 'tableChangeCount128':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.tableChangeCount128 != newValue) {
              nt$.value = nt$.value.copyWith(tableChangeCount128: newValue);
              developer.log('tableChangeCount128_received: $newValue');
            }
            break;

          case 'tableChangeCount64':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.tableChangeCount64 != newValue) {
              nt$.value = nt$.value.copyWith(tableChangeCount64: newValue);
              developer.log('tableChangeCount64_received: $newValue');
            }
            break;

          case 'tableChangeCount32':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.tableChangeCount32 != newValue) {
              nt$.value = nt$.value.copyWith(tableChangeCount32: newValue);
              developer.log('tableChangeCount32_received: $newValue');
            }
            break;

          case 'tableChangeCount':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.tableChangeCount != newValue) {
              nt$.value = nt$.value.copyWith(tableChangeCount: newValue);
              developer.log('tableChangeCount_received: $newValue');
            }
            break;

          case 'rangeStart':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.rangeStart != newValue) {
              nt$.value = nt$.value.copyWith(rangeStart: newValue);
              developer.log('rangeStart_received: $newValue');
            }
            break;

          case 'rangeEnd':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.rangeEnd != newValue) {
              nt$.value = nt$.value.copyWith(rangeEnd: newValue);
              developer.log('rangeEnd_received: $newValue');
            }
            break;

          case 'startBit':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.startBit != newValue) {
              nt$.value = nt$.value.copyWith(startBit: newValue);
              developer.log('startBit_received: $newValue');
            }
            break;

          case 'playingBit':
            final newValue = int.tryParse(value ?? '0') ?? 0;
            if (nt$.value.playingBit != newValue) {
              nt$.value = nt$.value.copyWith(playingBit: newValue);
              developer.log('playingBit_received: $newValue');
            }
            break;

          case 'isSwitched_32_64_128':
            final newValue = int.tryParse(value ?? '32') ?? 32;
          ...
          case 'GETNOTIFIERREQUEST':
            ...
            break;
                        case 'CHANGETABLEVIEW':
                            final newValue = int.tryParse(value ?? '0') ?? 0;
                            final newValue2 = int.tryParse(value2 ?? '0') ?? 0;
                            final newValue3 = (value3?.toLowerCase() == 'true');
                        // if ( TMPi != newValue) {
                         if ( newValue == newValue) {
                          TMPi = newValue;
                          developer.log('i_received: $newValue iStarts_received: $newValue2 dontChangeView_received: $newValue3');
                    await changeTableView(TMPi, newValue2, newValue3);
                 //       changeTableView(TMPi, newValue2, newValue3);
                        }
                          break;
           case 'SETNOTIFIERREQUEST':
           ...
           case 'SETSTATE': setState(() {}); break;
                  ...
                  // ntTblNotifier.notifyListeners();      // Solution:   Notify Listeners instead of nt$.value = ntTblNotifier !!!
                });
   //    //
...
//
                                  ...
                                  //**********************************************************************//
                                      ...
                                      if(TCC == 1) {iEndsTmp = ((rEND)/2).round() + 0;} else {iEndsTmp = ((rEND)/2).round() + 0 + (TCC - 1) * (nt$.value.nTcolS!*2);}
                                  ...
                                  //--------------------------------------- Main Cycle Loops Begin ---------------------------------------//
                                      ...
                                      for (int i = iStarts; i < iEnds; i++) { // traversing a list from start to finish //"mS" could be changed at any time by Slider
                                        ...
                                        while((nt$.value.onTapCollision1 == 1 || nt$.value.onTapCollision2 == 1 || nt$.value.onTapCollision3 == 1)  && sw.elapsedMilliseconds < 625)
                                        {
                                          // NO ENDLESS LOOP !!!  2 SECONDS
                                        }
                                        //
                                        
                                        TCC    = nt$.value.tableChangeCount!;    // try to fix incorrect view change after bothTables onTap
                                        if(TCC == 1) {iEndsTmp = ((nt$.value.rangeEnd!)/2).round() + 0;} else {iEndsTmp = ((nt$.value.rangeEnd!)/2).round() + 0 + (TCC - 1) * (nt$.value.nTcolS!*2);}  // use listenable value nt$.value.rangeEnd!
                                  ...
                                  //
                                        ...
                                        nt$.value.copyWith(playingBit:  PBIT);
                                        // nt$.value = ntTblNtfrsList;        // instead of previous value attach
                                  ...
                                  //////////////////////////////////////////////////////////////////// // range WAS all the way to the left and right (not set), and NOW it changed by user, then Stop:
                                        if((OisLEFT==0 && nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2) != 0) || (OisRIGHT==0 && nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4) != 0)) {
                                          setState(() {toggleIcnMsrBtn = true;});
                                          nt$.value.copyWith(msrTgl: 0);
                                          setState(() {
                                            isSwitched_32_64_128 = nt$.value.listener3264128!;
                                  ...
                                  /////////////////////////////////////////////////////////////////////////////////////////
                                        if (toggleIcnMsrBtn) {  // if measure button pressed by user
                                          ...
                                          if (fromTheBegin) {
                                            ...
                                            nt$.value.copyWith(startBit:           0);
                                            nt$.value.copyWith(playingBit: 0);
                                            nt$.value.copyWith(tableChangeCount:  1);
                                            nt$.value.copyWith(msrTgl: 0);
                                            nt$.value.copyWith(tableChangeCount32: 1);
                                            nt$.value.copyWith(tableChangeCount64: 1);
                                            nt$.value.copyWith(tableChangeCount128:  1);
                                            ...
                                            nt$.value.copyWith(playingBit: maxLength);
                                            //
                                            setState(() {toggleIcnMsrBtn = true;oneTraversingInstanceLaunched = false;});
                                  ...
                                  //////////////////////////// Current Bit Traversal by "j", PlayingNotes ////////////////////////////////
                                        for (int j = 1; j <= notesByBit; j++) {     // (j) is number of playing string at the moment, and  shortOrLong - is variant of note's length // <=   <=   <=  less or equal
                                          if (csvLst[i][j] != "") {                 // for simple Lists Use "add" method!!          // shortOrLongNum = 1 or 2 (Long|Short)  // You not to have to escape "asterisk", or "\" an "raw"
                                            ...
                                            if(nt$.value.msrTgl == 0) {noteVolumeBack = noteVolume; noteVolume = 0.0;} else {}           // to (iEnds - 1) note will not hear    1 of 2
                                            playSound(selectedtuningNum, j, shortOrLongNum, noteVolume, extension);  // sounds from here !
                                            if(nt$.value.msrTgl == 0) {noteVolume = noteVolumeBack;} else {}   //restoring normal Vol    // to (iEnds - 1) note will not hear    2 of 2
                                  ...
                                  //////////////////////////// End Current Bit Traversal by "j", PlayingNotes ////////////////////////////
                                        ...
                                        while((nt$.value.onTapCollision1 == 1 || nt$.value.onTapCollision2 == 1 || nt$.value.onTapCollision3 == 1)  && sw1.elapsedMilliseconds < 625)
                                        {
                                          // NO ENDLESS LOOP !!!  2 SECONDS
                                        }
                                        
                                          if(nt$.value.onTapCollision1 == 0 && nt$.value.onTapCollision2 == 0 && nt$.value.onTapCollision3 == 0) {
                                            if (shouldChangeView == true) {
                                              await changeTableView(i, iStarts, dontChangeView);  // await added!
                                            } else {} // instead of 256 "nt$.value.nTcolS!" replace to  "ntTblNtfrsList[21]['nt$.value.nTcolS!']" . It seems impossible!
                                          } else {} //  end onTap collision prevention
                                        //
                                        //
                                        if(OisLEFT == 0 && OisRIGHT == 0 && rSTART?.round() != 2*nt$.value.nTcolS!) {
                                          iEnds = maxLength;
                                        ...
                                        if((i+1).remainder(nt$.value.nTcolS!*2) == 0) {dontChangeView = false;} else {} //end if // don't change table view after manually toggle button play
                                        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////// iStarts, iEnds = depends on selected Range: //////////////////////////////////////////////////////////////////
    if (OisLEFT == 0 && OisRIGHT == 0 && rSTART?.round() != 2*nt$.value.nTcolS!)  {
      ...
      if(TCC == 1) { //first view (tableChangeCount=1)
        ...
        if(OisLEFT == 0) { // rangeStart is all the way to the left
          iStarts = ((rSTART)/2).round() + 0 + (TCC - 1) * (nt$.value.nTcolS!*2);
          PBIT = iStarts - 0;
          iEnds = ((rEND)/2).round() + 0 + (TCC - 1) * (nt$.value.nTcolS!*2);
        } else { // left range is NOT all the way to the left
          iStarts = ((rSTART)/2).round() + 0 + (TCC - 1) * (nt$.value.nTcolS!*2);
          PBIT = iStarts - 0;
          iEnds = ((rEND)/2).round() + 0 + (TCC - 1) * (nt$.value.nTcolS!*2);
    ...
    if(rSTART?.round() == 2*nt$.value.nTcolS!) {iStarts = SBIT + nt$.value.nTcolS! + 0;} else {}//end if Start: 64
...
/////////////////////////////////////////// additional second list traversal, range was extended while playing by user:
    ...
    if(TCC == 1) {iEndsTmp1 = ((nt$.value.rangeEnd!)/2).round() + 0;} else {iEndsTmp1 = ((nt$.value.rangeEnd!)/2).round() + 0 + (TCC - 1) * (nt$.value.nTcolS!*2);}
    if (iEndsTmp1 > iEnds) { // addtitional list traversal if the range is dynamically extended further by the user
      if (OisLEFT != 0 || OisRIGHT != 0) { // only if range was set
        iStarts = iEnds + 0; iEnds = iEndsTmp1; // use only listenable value !!! nt$.value.rangeEnd!
/////////////////////////////////////////// see rngExtend=true:
...
//
////////////////////// Completion naturally //////////////////////
      if(nt$.value.tableChangeCount!*2-1 == requiredMeasToStop) {i = iEnds - 1;} else {}
      if (i == iEnds - 1) {      // completion naturally upon reaching iEnds OR at the end of the List
...
//
  tablesBothOnTap  (bool isLongSwitching, bool isFromRangeSlider) async { // IF ASYNC FUNCTION NOT FINISHED AT THE MOMENT, IT WILL RETURN RANDOM VALUES !!! CHECK ALL AWAITS !!!
        // The solution to resolve Incorrect table view change dynamically while playback is to Use this line
    var sw = Stopwatch()..start();
    while(nt$.value.onTapCollision1 == 1 && sw.elapsedMilliseconds < 625)
    {
      // NO ENDLESS LOOP !!!  2 SECONDS
    }
    nt$.value.copyWith(onTapCollision2: 1); // nt$.value = ntTblNtfrsList;
             // The solution to resolve Incorrect table view change dynamically while playback is to Use this line
...
//**********************************************************************//
//
    // print('${nt$.value.tableChangeCount64!}   ${nt$.value.tableChangeCount128!*2}');
    ////////////////  tapMadeOnUpperOrLowerTable - at the moment when cursor ON IT        64=>128       128=>64  128=>64       64=>32  64=>32        32=>64
    if(isLongSwitching ==false && nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 64128){
      tapMadeOnUpperOrLowerTable = '064=>128';
      ////////////////////////////////////////////////////// FIX 0: //////////////////////// ( > ) ////////////////////  fix 0 NOT works, fix 1 works fine, fix 2 works fine    at extreme high tempo
      // if(nt$.value.tableChangeCount64! > nt$.value.tableChangeCount128!*2
      //     && nt$.value.tableChangeCount128! >1
      // ) {nt$.value.copyWith(tableChangeCount128:  nt$.value.tableChangeCount128! + 1);
      // nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount32! + 2);} else {}
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount128!);
      nt$.value.copyWith(rangeStart64bak:  nt$.value.rangeStart!);
      nt$.value.copyWith(rangeEnd64bak:  nt$.value.rangeEnd!);
      nt$.value.copyWith(rangeStart:  nt$.value.rangeStart128bak!);
      nt$.value.copyWith(rangeEnd:  nt$.value.rangeEnd128bak!);
      nt$.value.copyWith(nTcolS:  nTTcolSDouble);
        if(nt$.value.tableChangeCount! ==1) {
          nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS! * 2) + 0);
        } else {
          nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS! * 2) + 0);}
      nt$.value.copyWith(captionVariant0: 2); // '〇〇⬤'
      nt$.value.copyWith(listener3264128:  128);  // 32  64  128
      nt$.value.copyWith(rngEndSlider:  4*nt$.value.nTcolS!);
// strTxtNotifierList[0]['ntTableCaptionTxt'] = '${nt$.value.tableChangeCount}  ${nt$.value.startBit}';
// strTxtNotifier.value = strTxtNotifierList;
    }
    else if(isLongSwitching ==true && nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 3264){
      tapMadeOnUpperOrLowerTable = '064=>128';
      // if()
      // {nt$.value.copyWith(tableChangeCount128: ); }
      // else {nt$.value.copyWith(tableChangeCount128: ); }
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount128!);
      nt$.value.copyWith(rangeStart64bak:  nt$.value.rangeStart!);
      nt$.value.copyWith(rangeEnd64bak:  nt$.value.rangeEnd!);
      nt$.value.copyWith(rangeStart:  nt$.value.rangeStart128bak!);
      nt$.value.copyWith(rangeEnd:  nt$.value.rangeEnd128bak!);
      nt$.value.copyWith(nTcolS:  nTTcolSDouble);
      nt$.value.copyWith(listener326464128:  64128);
      nt$.value.copyWith(captionVariant0:  2); // '〇〇⬤'
      nt$.value.copyWith(listener3264128:  128);  // 32  64  128
      nt$.value.copyWith(rngEndSlider:  4*nt$.value.nTcolS!);
// strTxtNotifierList[0]['ntTableCaptionTxt'] = '${nt$.value.tableChangeCount}  ${nt$.value.startBit}';
// strTxtNotifier.value = strTxtNotifierList;
    ...
    else if(isLongSwitching ==false && nt$.value.listener3264128! ==128 && (((nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1) <= nt$.value.playingBit! && nt$.value.playingBit! <= (nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*2)) || nt$.value.rangeStart! >= nTTcolSDouble)){
      tapMadeOnUpperOrLowerTable = '128LOW=>064';
      ////////////////////////////////////////////////////// FIX 1: //////////////////////// ( < ) ////////////////////
      if(!isFromRangeSlider) {
        if(nt$.value.tableChangeCount64! < nt$.value.tableChangeCount128!*2) {
          nt$.value.copyWith(tableChangeCount64: nt$.value.tableChangeCount64! + 1);
          nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount32! + 2);} else {};
      } else {} // end isFromRangeSlider
      //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount64!);
      nt$.value.copyWith(rangeStart128bak:  nt$.value.rangeStart!);
      nt$.value.copyWith(rangeEnd128bak:  nt$.value.rangeEnd!);
      nt$.value.copyWith(rangeStart:  nt$.value.rangeStart64bak!);
      nt$.value.copyWith(rangeEnd:  nt$.value.rangeEnd64bak!);
      nt$.value.copyWith(nTcolS:  nTTcolSN);
      nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2));
      nt$.value.copyWith(captionVariant0:  1); // '〇⬤〇'
      nt$.value.copyWith(listener3264128:  64);  // 32  64  128
      nt$.value.copyWith(rngEndSlider:  4*nt$.value.nTcolS!);
// strTxtNotifierList[0]['ntTableCaptionTxt'] = '${nt$.value.tableChangeCount}  ${nt$.value.startBit}';
// strTxtNotifier.value = strTxtNotifierList;
    }
    else if(isLongSwitching ==false && nt$.value.listener3264128! ==128 && (((nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*0) <= nt$.value.playingBit! && nt$.value.playingBit! < (nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1)) || nt$.value.rangeStart! < nTTcolSDouble)){
      ...
      nt$.value.copyWith(nTcolS:  nTTcolSN);
      if(nt$.value.tableChangeCount! ==1) {
        nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + 0);
      } else {
        nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + 0);}
      nt$.value.copyWith(captionVariant0:  1); // '〇⬤〇'
      nt$.value.copyWith(listener3264128:  64);  // 32  64  128
...
// strTxtNotifier.value = strTxtNotifierList;
    ...
    else if(isLongSwitching ==false && nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 3264 && (((nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1) <= nt$.value.playingBit! && nt$.value.playingBit! <= (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*2)) || nt$.value.rangeStart! >= nTTcolSN)){
      tapMadeOnUpperOrLowerTable = '064LOW=>032';     //// PlayingBit is at Second (Lower) Row of 64 view
        ////////////////////////////////////////////////////// FIX 3: //////////////////////// ( < ) ////////////////////
      if(!isFromRangeSlider) {
        if(nt$.value.tableChangeCount32! < nt$.value.tableChangeCount64!*2) {
          nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount32! + 1);} else {};
      } else {} // end isFromRangeSlider
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount32!);
      nt$.value.copyWith(rangeStart64bak:  nt$.value.rangeStart!);
      nt$.value.copyWith(rangeEnd64bak:  nt$.value.rangeEnd!);
      nt$.value.copyWith(rangeStart:  nt$.value.rangeStart32bak!);
      nt$.value.copyWith(rangeEnd:  nt$.value.rangeEnd32bak!);
      nt$.value.copyWith(nTcolS:  nTTcolSHalf);
        if(nt$.value.tableChangeCount! ==1) {
          nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount32! - 1) * (nt$.value.nTcolS!*2) + 0);
        } else {
          nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount32! - 1) * (nt$.value.nTcolS!*2) + 0);}
      nt$.value.copyWith(captionVariant0:  0); // '⬤〇〇'
      nt$.value.copyWith(listener3264128:  32);  // 32  64  128
      nt$.value.copyWith(rngEndSlider:  4*nt$.value.nTcolS!);
// strTxtNotifierList[0]['ntTableCaptionTxt'] = '${nt$.value.tableChangeCount}  ${nt$.value.startBit}';
// strTxtNotifier.value = strTxtNotifierList;
    }
    else if(isLongSwitching ==true && nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 64128 && (((nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1) <= nt$.value.playingBit! && nt$.value.playingBit! <= (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*2)) || nt$.value.rangeStart! >= nTTcolSN)){
      tapMadeOnUpperOrLowerTable = '064LOW=>032';     //// PlayingBit is at Second (Lower) Row of 64 view
      if(((nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*0)
          <= nt$.value.playingBit! && nt$.value.playingBit! <
          (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1)) || nt$.value.rangeStart! < nTTcolSN)
      {nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount64!*2 - 0);}
      else {nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount64!*2 + 1);}
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount32!);
      nt$.value.copyWith(rangeStart64bak:  nt$.value.rangeStart!);
      nt$.value.copyWith(rangeEnd64bak:  nt$.value.rangeEnd!);
      nt$.value.copyWith(rangeStart:  nt$.value.rangeStart32bak!);
      ...
      if(nt$.value.tableChangeCount! ==1) {
        ...
        nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount32! - 1) * (nt$.value.nTcolS!*2) + 0);}
      nt$.value.copyWith(listener326464128:  3264);
      nt$.value.copyWith(captionVariant0:  0); // '⬤〇〇'
      nt$.value.copyWith(listener3264128:  32);  // 32  64  128
      nt$.value.copyWith(rngEndSlider:  4*nt$.value.nTcolS!);
// strTxtNotifierList[0]['ntTableCaptionTxt'] = '${nt$.value.tableChangeCount}  ${nt$.value.startBit}';
// strTxtNotifier.value = strTxtNotifierList;
    ...
    else if(isLongSwitching ==false && nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 3264 && (((nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*0) <= nt$.value.playingBit! && nt$.value.playingBit! < (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1)) || nt$.value.rangeStart! < nTTcolSN)) {
      tapMadeOnUpperOrLowerTable = '064UP=>032';
        ////////////////////////////////////////////////////// FIX 4: //////////////////////// ( >= ) ////////////////////
      if(!isFromRangeSlider) {
        if(nt$.value.tableChangeCount32! >= nt$.value.tableChangeCount64!*2
            && nt$.value.tableChangeCount64! >1
        ) {nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount32! - 1);} else {}
      } else {} // end isFromRangeSlider
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount32!);
...
// strTxtNotifier.value = strTxtNotifierList;
    ...
    else if(isLongSwitching ==true && nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 64128 && (((nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*0) <= nt$.value.playingBit! && nt$.value.playingBit! < (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1)) || nt$.value.rangeStart! < nTTcolSN)) {
      tapMadeOnUpperOrLowerTable = '064UP=>032';
      if(((nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*0) <= nt$.value.playingBit! && nt$.value.playingBit! < (nt$.value.tableChangeCount64! - 1) * (nt$.value.nTcolS!*2) + (nt$.value.nTcolS!*1)) || nt$.value.rangeStart! < nTTcolSN)
      {nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount64!*2 - 1);}
      else {nt$.value.copyWith(tableChangeCount32: nt$.value.tableChangeCount64!*2);}
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount32!);
      nt$.value.copyWith(rangeStart64bak:  nt$.value.rangeStart!);
      nt$.value.copyWith(rangeEnd64bak:  nt$.value.rangeEnd!);
      nt$.value.copyWith(rangeStart:  nt$.value.rangeStart32bak!);
...
// strTxtNotifier.value = strTxtNotifierList;
    ...
    else if (isLongSwitching ==false && nt$.value.listener3264128! ==32 && nt$.value.listener326464128! == 3264) {                                         //// Current View is "32"
      tapMadeOnUpperOrLowerTable = '032=>064';
      nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount64!);
      nt$.value.copyWith(rangeStart32bak: nt$.value.rangeStart!);
      nt$.value.copyWith(rangeEnd32bak: nt$.value.rangeEnd!);
      nt$.value.copyWith(rangeStart:  nt$.value.rangeStart64bak!);
      nt$.value.copyWith(rangeEnd:  nt$.value.rangeEnd64bak!);
      nt$.value.copyWith(nTcolS:  nTTcolSN);
      if(nt$.value.tableChangeCount! ==1) {
...
//**********************************************************************//
    ...
    nt$.value.copyWith(onTapCollision2: 0); // nt$.value = ntTblNtfrsList;
    //
  } // End tablesBothOnTap()
//
...
//**********************************************************************//
    nt$.value.copyWith(rngEndSlider:  nt$.value.nTcolS!*4);
    // nt$.value = ntTblNtfrsList;
//**********************************************************************//
//     setState(() {});  // needed!!!
    //
...
//
  switchingTableViewByLongPress() async {
        // The solution to resolve Incorrect table view change dynamically while playback is to Use this line
    var sw = Stopwatch()..start();
    while((nt$.value.onTapCollision1 == 1 || nt$.value.onTapCollision2 == 1)  && sw.elapsedMilliseconds < 625)
    {
      // NO ENDLESS LOOP !!!  2 SECONDS
    }
    nt$.value.copyWith(onTapCollision3: 1); // nt$.value = ntTblNtfrsList;
        // The solution to resolve Incorrect table view change dynamically while playback is to Use this line
//**********************************************************************//
//**********************************************************************//

    // if(nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 3264){
    //   nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount128!);
    //   nt$.value.copyWith(rangeStart64bak:  nt$.value.rangeStart!);
    //   nt$.value.copyWith(rangeEnd64bak:  nt$.value.rangeEnd!);
    //   nt$.value.copyWith(rangeStart:  nt$.value.rangeStart128bak!);
    //   nt$.value.copyWith(rangeEnd:  nt$.value.rangeEnd128bak!);
    //   nt$.value.copyWith(listener326464128:  64128);
    //   nt$.value.copyWith(listener3264128:  128);
    // if(nt$.value.tableChangeCount! ==1) {nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS! * 2) + 0);} else {nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount128! - 1) * (nt$.value.nTcolS! * 2) + 0);}
    //   nt$.value.copyWith(captionVariant0:  2); // '〇〇⬤'
    // }
    // else if(nt$.value.listener3264128! ==64 && nt$.value.listener326464128! == 64128) {
    //   nt$.value.copyWith(tableChangeCount:  nt$.value.tableChangeCount32!);
    //   nt$.value.copyWith(rangeStart64bak:  nt$.value.rangeStart!);
    //   nt$.value.copyWith(rangeEnd64bak:  nt$.value.rangeEnd!);
    //   nt$.value.copyWith(rangeStart:  nt$.value.rangeStart32bak!);
    //   nt$.value.copyWith(rangeEnd:  nt$.value.rangeEnd32bak!);
    //   nt$.value.copyWith(listener326464128:  3264);
    //   nt$.value.copyWith(listener3264128:  32);
    // if(nt$.value.tableChangeCount! ==1) {nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount32! - 1) * (nt$.value.nTcolS!*2) + 0);} else {nt$.value.copyWith(startBit:  (nt$.value.tableChangeCount32! - 1) * (nt$.value.nTcolS!*2) + 0);}
    //   nt$.value.copyWith(captionVariant0:  0); // '⬤〇〇'
    // }
    // else {}
...
//**********************************************************************//
 ...
 // await tablesBothOnTap(false); await tablesBothOnTap(false); // 1-st and 2-nd tap  //  No Needed,    for view indicator changes immidiately and ... correct view change???
    // setState(() {});
    await setDataSharedPref();   // CALL
    nt$.value.copyWith(onTapCollision3:  0); // nt$.value = ntTblNtfrsList;
                                ...
                                child:Align(
                                  ...
                                  child:
                                      Row(
                                        children:
                                            [
                                              ValueListenableBuilder<NtState> (
                                                  valueListenable: nt$,
                                                  builder: (context, _, __) {
                                                    int  tmpidx  = nt$.value.captionVariant0!;              // Null check on Null value: if you make a mistake in [22] or [23]
                                                    int  tmpidx1 = nt$.value.captionVariant1!;
                                                    int  fontIndicator = 0;
...
//////// FIRST: ///////////////////// toDo: variable doesn't have to be inside setState(), widget rebuilds with new value of variable after each setState WITHOUT ValueListenableBuilder!!!, even if variable is not inside the setState() !!!
                                    ValueListenableBuilder<NtState> (     // works very well!
                                        valueListenable: nt$,
                                        builder: (context, _, __) {  //As few assignments of new variables as possible! It slows down!
                                          switch(nt$.value.listener3264128!) {    // linear dependence of font size on device screen width and XY-shift on device screen width:
                                            case 32:
                                          ...
                                          return Table(
                                              ...
                                              [
                                                for(int i = 0; i <= stringsNum; i++)  TableRow(children: [    // was i < stringsNum     fix: Note F2 was not shown in Table   1 of 2
                                                  for(int j = nt$.value.startBit!.round(); j! < nt$.value.startBit!.round() + nt$.value.nTcolS!; j++)
// if(data1[i][j] != '') {data1[i][j] = '⬤';} else {}  // replacing all notes with '⬤', filled circle symbol
                                                    if (data1[i][j] != '' && j==nt$.value.playingBit! && nt$.value.msrTgl == 1) ...[
                                                    ...
                                                    ] else if(nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2) == 0 && nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4) == 0 && j!=nt$.value.playingBit) ...[
                                                      Container(
                                                    ...
                                                    ] else if(nt$.value.rangeStart?.round() != nt$.value.rangeEnd?.round() && (((nt$.value.rangeStart?.round())!/2 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) == j && (nt$.value.tableChangeCount! == 1 || nt$.value.tableChangeCount! > 3)) || ((nt$.value.rangeStart?.round())!/2 + 0 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) == j && (nt$.value.tableChangeCount! == 2 || nt$.value.tableChangeCount! == 3)))  && j!=nt$.value.playingBit!) ...[
                                                      Container(                  // Border-L Green Range 1st table
                                                        decoration: BoxDecoration(
                                                          color:   themeappLightDark==1 ? Colors.lime[100] : invertCustom(Colors.lime[100]!),
                                                          border: Border(
                                                            left: BorderSide(color: themeappLightDark==1 ? Color(0xffffce4c) : invertCustom(Color(0xffffce4c)),width: nt$.value.listener3264128! ==32 ? 2 : 1,),    // width (3) OR width (1) Depends On Columns Number (True 32 /False 64)
                                                          ),
                                                        ),                      // Border-L Green Range 1st table (now it's thin Orange)
                                                        child:   Text(' ', style: TextStyle(fontSize: heightSupportingSize)),
                                                      ),
                                                    ] else if(nt$.value.rangeStart?.round() != nt$.value.rangeEnd?.round() && (((nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) == j && (nt$.value.tableChangeCount! == 2 || nt$.value.tableChangeCount! == 3)) || ((nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) == j && (nt$.value.tableChangeCount! == 1 || nt$.value.tableChangeCount! > 3)))  && j!=nt$.value.playingBit!) ...[
                                                      Container(            // Border-R Green Range 1st table
                                                        decoration: BoxDecoration(
                                                          color:   themeappLightDark==1 ? Colors.lime[100] : invertCustom(Colors.lime[100]!),
                                                          border: Border(
                                                            right: BorderSide(color: themeappLightDark==1 ? Color(0xffffce4c) : invertCustom(Color(0xffffce4c)),width: nt$.value.listener3264128! ==32 ? 2 : 1,),    // width (3) OR width (1) Depends On Columns Number (True 32 /False 64)
                                                          ),
                                                        ),                  // Border-R Green Range 1st table (now it's thin Orange)
                                                        child:   Text(' ', style: TextStyle(fontSize: heightSupportingSize)),
                                                      ),
                                                    ] else if((nt$.value.rangeStart?.round())!/2 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2)   <= j && j <= (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) && j!=nt$.value.playingBit && nt$.value.tableChangeCount! == 1) ...[
                                                    ...
                                                    ] else if(nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2) == 0 && nt$.value.rangeStart?.round() != nt$.value.nTcolS!*2 && nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4) != 0 && j < (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2)  && j!=nt$.value.playingBit && nt$.value.tableChangeCount! == 1) ...[
                                                    ...
                                                    ] else if((nt$.value.rangeStart?.round())!/2 + 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2)   <= j && j <= (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) && j!=nt$.value.playingBit && nt$.value.tableChangeCount! == 2) ...[
                                                      Container(                      // Green Range 1st table (see also: second cond.)
                                                        color:   themeappLightDark==1 ? Colors.lime[100] : invertCustom(Colors.lime[100]!),    // S + 1 + (C-1)*(32*2) < = j < = E + 0 + (C-1)*(32*2) && j: is Not PlayingBit
                                                        child:   Text(' ', style: TextStyle(fontSize: heightSupportingSize)),  // small text color is the same as the cell colour, text will not be visible
                                                      ),
                                                    ] else if(nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2) == 0 && nt$.value.rangeStart?.round() != nt$.value.nTcolS!*2 && nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4) != 0 && j < (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2)  && j!=nt$.value.playingBit && nt$.value.tableChangeCount! == 2) ...[
                                                      Container(                      // Green Range 1st table (second condition)
                                                        color:   themeappLightDark==1 ? Colors.lime[100] : invertCustom(Colors.lime[100]!),    // S: divides by 64 && S != 64 + (C-1)*(32*4) (Not at the Middle Position) && E: Not divides by 128 && j < E + (C-1)*(32*2)
                                                        child:   Text(' ', style: TextStyle(fontSize: heightSupportingSize)),  // small text color is the same as the cell colour, text will not be visible
                                                      ),
                                                    ] else if((nt$.value.rangeStart?.round())!/2 + 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2)   <= j && j <= (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) && j!=nt$.value.playingBit && nt$.value.tableChangeCount! == 3) ...[
                                                      Container(                      // Green Range 1st table (see also: second cond.)
                                                        color:   themeappLightDark==1 ? Colors.lime[100] : invertCustom(Colors.lime[100]!),    // S + 1 + (C-1)*(32*2) < = j < = E + 0 + (C-1)*(32*2) && j: is Not PlayingBit
                                                        child:   Text(' ', style: TextStyle(fontSize: heightSupportingSize)),  // small text color is the same as the cell colour, text will not be visible
                                                      ),
                                                    ] else if(nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2) == 0 && nt$.value.rangeStart?.round() != nt$.value.nTcolS!*2 && nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4) != 0 && j < (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2)  && j!=nt$.value.playingBit && nt$.value.tableChangeCount! == 3) ...[
                                                      Container(                      // Green Range 1st table (second condition)
                                                        color:   themeappLightDark==1 ? Colors.lime[100] : invertCustom(Colors.lime[100]!),    // S: divides by 64 && S != 64 + (C-1)*(32*4) (Not at the Middle Position) && E: Not divides by 128 && j < E + (C-1)*(32*2)
                                                        child:   Text(' ', style: TextStyle(fontSize: heightSupportingSize)),  // small text color is the same as the cell colour, text will not be visible
                                                      ),
                                                    ] else if((nt$.value.rangeStart?.round())!/2 + 0 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2)   <= j && j <= (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) && j!=nt$.value.playingBit && nt$.value.tableChangeCount! > 3) ...[
                                                      Container(                      // Green Range 1st table (see also: second cond.)
                                                        color:   themeappLightDark==1 ? Colors.lime[100] : invertCustom(Colors.lime[100]!),    // S + (C-1)*(32*2) < = j < = E - 1 + (C-1)*(32*2) && j: is Not PlayingBit
                                                        child:   Text(' ', style: TextStyle(fontSize: heightSupportingSize)),  // small text color is the same as the cell colour, text will not be visible
                                                      ),
                                                    ...
                                                    ] else if(j==nt$.value.playingBit! && nt$.value.msrTgl == 1 && nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2) == 0 && nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4) == 0) ...[
                                                      Container(
                                                    ...
                                                    ] else if(j==nt$.value.playingBit! && nt$.value.msrTgl == 1) ...[
                                          ...
                                          return Table(
                                              ...
                                              [
                                                for(int i = 0; i <= stringsNum; i++)  TableRow(children: [    // was i < stringsNum     fix: Note F2 was not shown in Table   2 of 2
                                                  for(int j = nt$.value.startBit!.round() + nt$.value.nTcolS!; j! < nt$.value.startBit!.round() + nt$.value.nTcolS!*2; j++)
// if(data1[i][j] != '') {data1[i][j] = '⬤';} else {}  // replacing all notes with '⬤', filled circle symbol
                                                    if (data1[i][j] != '' && j==nt$.value.playingBit! && nt$.value.msrTgl == 1) ...[
                                                    ...
                                                    ] else if(nt$.value.rangeStart?.round().remainder(nt$.value.nTcolS!*2) == 0 && nt$.value.rangeStart?.round() != nt$.value.nTcolS!*2 && nt$.value.rangeEnd?.round().remainder(nt$.value.nTcolS!*4) == 0 && j!=nt$.value.playingBit) ...[
                                                      Container(
                                                    ...
                                                    ] else if(nt$.value.rangeStart?.round() != nt$.value.rangeEnd?.round() && (((nt$.value.rangeStart?.round())!/2 + 0 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) == j && (nt$.value.tableChangeCount! == 1 || nt$.value.tableChangeCount! > 3)) || ((nt$.value.rangeStart?.round())!/2 + 0 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) == j && (nt$.value.tableChangeCount! == 2 || nt$.value.tableChangeCount! == 3)))  && j!=nt$.value.playingBit!) ...[
                                                    ...
                                                    ] else if((nt$.value.rangeStart?.round())!/2 + 0 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) <= j && j <= (nt$.value.rangeEnd?.round())!/2 - 1 + (nt$.value.tableChangeCount! - 1) * (nt$.value.nTcolS!*2) && j!=nt$.value.playingBit && nt$.value.tableChangeCount! == 1) ...[
                                  ...
                                  Stack(
                                    children: <Widget>[                 // Container is OVER the painting, because it is LAST in the List
                                      ...
                                      ValueListenableBuilder<NtState> (
                                          valueListenable: nt$,
                                          builder: (context, _, __) {
                                            double? doubleVar1 = (nt$.value.rangeStart)?.toDouble();   //to prevent Null check on Null value
                                            double? doubleVar2 = (nt$.value.rangeEnd)?.toDouble();
                                            return RangeSlider(
                                              values: RangeValues(doubleVar1!, doubleVar2!),
                                              /////////////////////////////////////////////////////////////////////////////
                                              //       // max: (4*nt$.value.nTcolS!).toDouble(),   // 128        // try to fix incorrect view change after tables both onTap
                                              //       max: (rngEnd).toDouble(),
                                              //       // divisions: 2*(nt$.value.nTcolS!.toInt()),         // 64
                                              //     //divisions: (rngEnd/2).round(),   // works fine
                                              //     //divisions: (rngEnd/2).toInt(),
                                              //       divisions: (rngEnd~/2),          // ~~~~~~~~~~~~//////////// divide evenly    ~/      (~)
                                              //////////////////////////////////////////////////////////////////////////////
                                                  // max: (4*nt$.value.nTcolS!).toDouble(),   // 128
                                                  max: (nt$.value.rngEndSlider)!.toDouble(),
                                                  // divisions: 2*nt$.value.nTcolS!,          // 64
                                                  divisions: ((nt$.value.rngEndSlider)!/2).toInt(),
                                              //////////////////////////////////////////////////////////////////////////////
                                              ...
                                              //min: isSwitched_32_64_128==64 ? 0.0 : 0.0, // allways zero: if mode is 64 or not
                                                min: 0.0,
                                              labels: RangeLabels(
                                                (nt$.value.rangeStart!).toString(),
                                                (nt$.value.rangeEnd!).toString(),
                                              ),
                                              onChanged: (RangeValues values1) {
                                                cnslDelay1Ntfr.value = true;
                                                // nt$.value.copyWith(rangeStart:  values1.start.round()); //works fine
                                                // nt$.value.copyWith(rangeEnd:  values1.end.round());
                                                nt$.value.copyWith(rangeStart:  values1.start.toInt());
                                                nt$.value.copyWith(rangeEnd:  values1.end.toInt());
                                                // nt$.value = ntTblNtfrsList;
                                        ...
                                        TextButton.icon(
                                          ...
                                          onPressed: (){
                                            ...
                                            if(isSwitchedMonitorFile==true && fromTheBegin==false) {
                                              ...
                                              nt$.value.copyWith(tableChangeCount64: 1);
                                              nt$.value.copyWith(tableChangeCount128:  1);
                                              nt$.value.copyWith(rangeEnd:  nt$.value.nTcolS!*4);  // resetting Range Slider
                                              // nt$.value = ntTblNtfrsList;
                                              setDataSharedPref();
...
//
                                ConstrainedBox(
                                  ...
                                  TextButton.icon(       //toDo: change play button style
                                    ...
                                    label: Align(
                                      alignment: Alignment.centerRight,
                                      child: ValueListenableBuilder<NtState> (     // Do not forget to write "return", otherwise you will get a red screen
                                          valueListenable: nt$,
                                          builder: (context, _, __) {
                                            ...
                                            return RichText(text: TextSpan(children: [
                                              TextSpan(text: '${(nt$.value.tableChangeCount!*2-1).toString().padLeft(3, '0')} - ${(maxNotEmptyPos / nt$.value.nTcolS!).ceil().toString().padLeft(3, '0')}', style: TextStyle(
                                ...
                                Align (
                                  ...
                                  Container(
                                    ...
                                    child: ValueListenableBuilder<NtState> (     // Do not forget to write "return", otherwise you will get a red screen
                                        valueListenable: nt$,
                                        builder: (context, _, __) {
                                          return Text('${nt$.value.tableChangeCount!*2-1}', style: TextStyle(fontSize: 58.0, color: themeappLightDark==1 ? Colors.black87 : invertCustom(Colors.black26)));
                                        }
                                    ),
                                  ),
                                  //Text(tempperformrsInfo, style: TextStyle(fontStyle: FontStyle.normal)),
