...
//   await SoLoud.instance.init();
      ...
      if (!SoLoud.instance.isInitialized) {
        ...
        } on SoLoudPlayerAlreadyInitializedException {} catch (e) {
...
// developer.log('Isolate Received: ${message.runtimeType} | Content: $message');
// developer.log(cachedFilesPaths.toString());
      //
     listTraversal (cachedFilesPaths);
});
//
} //end IsolateEntryPoint
//
//
Future<void> listTraversal (cachedFilesPaths) async {
  //
...
// //
  Future<void> playSound(int tuning, int number, int shortOrLong, double vol, int ext) async {
    ...
    switch (ext) {
      case 1: xts = 'wav'; pth = 'assets/wav/'; break;
      case 2: xts = 'm4a'; pth = 'assets/m4a/'; break;
      case 4: xts = 'mp3'; pth = 'assets/mp3/'; break;
      default: xts = 'wav'; pth = 'assets/wav/'; break;
    }
    if (tuning == 11) { tIdx = 2; pth = 'assets/wavn/'; }
    ...
    try {
      AudioSource? src;
      if (soundCache.containsKey(ful)) {
        src = soundCache[ful];
      } else {
        String? loc = cachedFilesPaths[ful];
        if (loc != null) {
          src = await SoLoud.instance.loadFile(loc);
          soundCache[ful] = src;
        }
      }
...
// //
  Future<void> playSound_(int tuning, int number, int shortOrLong, double nVol, int ext) async {
...
developer.log(cachedFilesPaths[j].toString());
